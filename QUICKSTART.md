...
> ⚠️ **Long Polling — только для разработки.** В production используйте Webhook: long polling ограничен по скорости и сроку хранения событий и может терять обновления под нагрузкой.
>
> ⚠️ **С 25.05.2026 webhook должен быть на HTTPS с сертификатом от доверенного CA.** HTTP и самоподписные сертификаты больше не поддерживаются.
...
1. Разместите `bot.php` на веб-сервере с **HTTPS** и сертификатом от доверенного CA (например, Let's Encrypt). Самоподписные сертификаты не подойдут.
2. Зарегистрируйте webhook через API (выполняется один раз отдельным скриптом):
...
Чтобы обновить подписку (например, изменить список событий или URL) — просто вызовите `createSubscription` ещё раз. Старую подписку с другим URL можно явно снять:
...
- `webhook-bot.php` - Production-режим: HTTPS webhook и регистрация подписки
